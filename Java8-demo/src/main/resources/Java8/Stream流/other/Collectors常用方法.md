...
```java
// hutool中对象转换工具类 
Map<String, Object> userMap = BeanUtil.beanToMap(user, "id", "phone", "nickName")  
		// Java8新特性：Stream
		.entrySet().stream()  
		.collect(Collectors.toMap(  
				Map.Entry::getKey, // 设置key  
				entry -> entry.getValue().toString() // 设置value  
		));  
```
...
```java
List<Integer> integers = Arrays.asList(1,2,3,4,5,6,6);
Set<Integer> newSet = integers.stream()
		.map(x -> x*x)
...
用于创建只读`Set`集合。任何试图对此不可修改`Set`集合进行更改的尝试都将导致`UnsupportedOperationException`。
...
## **Long类型集合的平均值：averagingLong()** 重要

查找`Long`类型集合的平均值。

注意：

返回的是Double类型而不是 Long类型
...
## **Integer类型集合的平均值：averagingInt()** 重要

查找Integer类型集合的平均值。

注意：

返回的是Double类型而不是 int类型
...
## **Double类型集合的平均值：averagingDouble()** 重要
...
## **Long求和:summingLong ()** 重要

与前两个相同，用于添加`long`值或`int`值。

可以对`int`值使用`summinglong()`，但不能对`long`值使用`summingInt()`。
